...
def DataSplit(hudRawData, PerformanceCalculationConditions, PerformanceData, PerformanceErrorData):
    for line in hudRawData:
        if PerformanceCalculationConditions["overlapCheckData"] != line[0]:
                ...
                # 메모리 데이터
                PerformanceData['memoryData'].append(line[2])
                ...
                for i in range(3, len(line)):
                    if ("<" not in line[i]):
                        if (line[i] != ""):
                            if i % 2 == 1:
                                # 성능 시간 데이터
                                PerformanceData["frameTimeData"].append(np.float32(line[i]))
                                
                            else:
                                # GPU 시간 데이터
                                PerformanceData["gpuTimeData"].append(np.float32(line[i]))
                    else:
                        if i % 2 == 1:
...
def ConverttoFPS(PerformanceData, PerformanceCalculationConditions, UnitConversion = 1000, DecimalPoint = 2):
    for i in range(len(PerformanceData["frameTimeData"])):
        PerformanceCalculationConditions["secondSum"] += np.float(PerformanceData["frameTimeData"][i])
...
# 파일 저장
PerformanceCsvSave("FPS-Result.csv", f"FPS - 약 {PerformanceCalculationConditions["benchmarkBasedTime"]} ms마다 평균치 계산", None)
PerformanceCsvSave("Frametime-Result.csv", f"Frametime", None)
PerformanceCsvSave("GPUTime-Result.csv", f"GPUTime", None)
PerformanceCsvSave("Memory-Result.csv", f"Memory(MB)", None)
PerformanceCsvSave("Frametime-Error.csv", f"Frametime error list", None)
PerformanceCsvSave("GPUTime-error.csv'", f"GPUTime error list", None)
